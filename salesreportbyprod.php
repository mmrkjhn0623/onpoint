...
{	
	...
	$this->Cell(0,0,'Order by: Product',3,0,'R');
	$this->Ln(3);
}
...
$result = mysqli_query($connString, "SELECT SUM(invoice.quantity * items.unit_price) as 'cost', items.item_name as 'item', SUM(invoice.quantity) as 'quantity', invoice.date as 'date' FROM items,invoice where items.item_id = invoice.item_id and invoice.company_id = '".$_SESSION["companyid"]."' and invoice.date >= '".$_GET["datefrom"]."' and invoice.date <= '".$_GET["dateto"]."' and invoice.quantity != '0' group by items.item_name ORDER BY `cost`  DESC") or die("database error:". mysqli_error($connString));
...
$pdf->Cell(43,10,'Quantity',1,0,'C');
$pdf->Cell(45,10,'Total Sale',1,0,'C');
...
$pdf->Ln();
$pdf->Cell(100,7,$row['item'],'L',0,'C');
$pdf->Cell(43,7,$row['quantity'],'L',0,'C');
$pdf->Cell(45,7,number_format($row['cost'],2),'LR',0,'C');
...
$pdf->Cell(45,10,number_format($totalamount,2),1,0,'C');
$pdf->Output();
?>